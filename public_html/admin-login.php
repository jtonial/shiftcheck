...
if (!isset($_SESSION['userid'])) {
	if (isset($_POST['email'])){
		$query="SELECT employer_id,password FROM employers WHERE login_name='".$email."' LIMIT 1";
		...
		if ($return && mysql_num_rows($return)){
			...
			$s['Employer']=$row['employer_id'];
			if ($_POST['password'] != NULL) {
				if (myhash($_POST['password']) == $row['password']) {
					//Signin validated.
					//Update signings, last logon_date, set failed_attempts to 0 in database
					//$query="UPDATE users SET last_login=NOW(), login_count=login_count+1 WHERE userid=".$row['userid']." LIMIT 1"; //This will be updated for tracking when the database columns for trackig are added
					//This info could be saved in an IonnoDB tracking table (faster insert)
					//echo "<br/><br/>".$query;
					//$return=mysql_query($query,$dbphp); 
			...
			$s['Employer']='Email Not Registered';
		}
