$(function() {
	...
	window.ShiftView = Backbone.View.extend({
		...
		tagName: 'div',
		className: 'schedule-row row-fluid clearfix',

		template: _.template($('#shift-template').html()),

		events: {
			"click .edit-shift" : "toggleEditOn",
			"click .save-shift" : "saveChanges"
		},

		toggleEditOn: function () {
			this.$('.btn.edit-shift').hide();
			this.$('.btn.save-shift').show();
			this.$('.ui-slider-handle').show();
		},
		saveChanges: function () {
			this.$('.btn.edit-shift').show();
			this.$('.btn.save-shift').hide();

			this.$('.ui-slider-handle').hide();
			console.log('saving...');
		},
		render: function () {
			$(this.el).html(this.template(this.model.toJSON()));
