...
require(__basePath+'/connections/logger').info('Loading schedule engine...');
...
function permissionCheck (entity) {
  return function (req, res, next) {
    ...
    if (obj.obj_type == 'schedule') {
      if (obj.method.toLowerCase() == "get") {
        if (req.schedule) {
          if (req.user.employer_id == req.schedule.get('employer_id')) {
            if (req.user.admin || req.schedule.get('published')) {
              next();
            } else {
        ...
        } else { // It is not a specific schedule
          next();
        }
      } else if (req.schedule) { // This will be a put or delete
        if (req.user.employer_id == req.schedule.get('employer_id')) {
          next();
        } else {
          Main.Render.code( req, res, { statusCode : 403 } );
    ...
    } else if (obj.obj_type == 'shift') {
      if (req.user.admin && (req.user.employer_id == req.schedule.get('employer_id'))) {
        next();
      } else {
        Main.Render.code( req, res, { statusCode : 403 } );
      }
