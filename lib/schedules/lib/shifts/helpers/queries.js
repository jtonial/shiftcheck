...
module.exports = queries = {
  ...
  'getShiftsBySchedulePopulated' : 'SELECT s.shift_id as id, s.start, s.end, s.position_id, s.employee_id, p.position, CONCAT(e.first_name, " ", e.last_name) as employee_name FROM shifts as s JOIN positions as p USING (position_id) JOIN users as e ON (employee_id = e.user_id) WHERE schedule_id = ?',
