        ...
        if (err) {
          ...
          if (row[0]) {
            ...
            if (Main.Helpers.calcHash(password, row[0].salt) == row[0].password) {
              ...
              db.query(Main.Queries.updateEmployeeLogin, [req.session.employee_id], function (err, numAffected) {
                if (err) {
...
exports.create = function (obj, cb) {
  ...
  db.query(Main.Queries.insertEmployee, [email, username, pass, salt, fname, lname, employer], cb)
};
exports.deactivate = function (obj, cb) {
  db.query(Main.Queries.deactivateEmployee, [obj.employee_id, obj.employer_id], cb)
};
...
exports.fetch = function (obj, cb, cb2) {
  ...
  db.query(Main.Queries.selectEmployee, [id], function (err, row) {
...
exports.getByEmployer = function (obj, cb) {
  ...
  db.query(Main.Queries.selectEmployees, [employer], function (err, rows) {
    if (err) {
      response.statusCode = 500;
      response.message = err.code;
      Main.Logger.error(err.code);
...
exports.changePassword = function (obj, cb) {
  ...
  db.query(Main.Queries.changeEmployeePassword, [newpassword, id, oldpassword], cb);
